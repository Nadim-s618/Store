...
export async function POST(request: Request) {
  try {
    ...
    const order = await prisma.order.findFirst({ where: { id: body.orderId, OR: [{ trackingCode: body.trackingCode }, ...(customer ? [{ userId: customer.id }] : [])] }, include: { items: { include: { product: { select: { name: true } } } } } })
    ...
    return Response.json({ error: 'The invoice could not be generated right now.' }, { status: 500 })
  }
}
