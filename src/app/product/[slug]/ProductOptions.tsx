...
type MeasurementField = { key: 'height' | 'width' | 'waist' | 'hip'; label: string }
type Measurement = { size: string; height: number | null; width: number | null; waist: number | null; hip: number | null }
const views = ['Front', 'Back', 'Right', 'Left']

export default function ProductOptions({ productId, productName, productPrice, variants, images, fallbackImage, reviews: initialReviews, relatedProducts, measurementFields, measurements }: { productId: string; productName: string; productPrice: number; variants: Variant[]; images: ProductImage[]; fallbackImage?: string; reviews: Review[]; relatedProducts: RelatedProduct[]; measurementFields: MeasurementField[]; measurements: Measurement[] }) {
  ...
  return <>
    ...
    <div className={styles.mobileSizeChart}><p className={styles.reviewKicker}>Find your fit</p><h2>Size chart.</h2><div className={styles.chartTable}><table><thead><tr><th>Size</th>{measurementFields.map((field) => <th key={field.key}>{field.label}</th>)}</tr></thead><tbody>{sizes.map((size) => { const measurement = measurements.find((item) => item.size === size); return <tr key={size}><td>{size}</td>{measurementFields.map((field) => { const value = measurement?.[field.key]; return <td key={field.key}>{value != null ? `${value.toFixed(1)} cm` : '—'}</td> })}</tr> })}</tbody></table></div><p className={styles.chartNote}>Measurements are shown in centimetres.</p></div>
